...
Kolmogorov Cycle Condition: for every cycle $x_0, x_1, x_2, \dots, x_n = x_0$, \boxed{\prod_{i=0}^{n-1} p(x_i, x_{i+1}) = \prod_{i=0}^{n-1} p(x_{i+1}, x_i)}. \\
...
\colorbox{red}{TODO: all linear markov chains are reversible, 3x3 KCC}


\subsection{Limit Distributions}
\colorbox{red}{TODO: perron frobenius, lim sum f(x) = sum f(x) pi(x)}
